...
function getImagen() {
  ...
  let usuarios = JSON.parse(localStorage.getItem("user"));
   ...
   let imagen = usuarios[posicionUsuario].img;
    console.log("Esta es mi imagen",imagen);
   let index = url.indexOf(imagen);
...
//si no hay ninguno guarda los usuarios fijos en el localStorage
...
//Función sencilla que valida si el telefono está en formato número 
//y si su longitud está entre 9 y 15
function validarTelefono(){
  let valor = document.getElementById("tel").value;
...
function agregarUsuario() {
  ...
  let password = document.getElementById("paso").value;
  let telefono = document.getElementById("tel").value;
  if (checkFormulario() && evitarDuplicados(email) && validarEmail(email) && validarTelefono()) {
    let usuarios = {
      nombre: nombre,
      email: email,
      img: img,
    ...
    //a cero al inicio, cuando llegue a 15 se reiniciará el contador.
    // indice++;
    // if (indice == 15) {
    //   indice = -1;
    //   indice++;
